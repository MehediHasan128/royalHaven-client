...
const PropertyDetails = ({ propertyId }: { propertyId: string | null }) => {
  ...
  return (
    <div>
      ...
      {/* Fav or save icon */}
      <div className="flex justify-end gap-3 p-3 text-2xl">
        <IoHeart
          onClick={() => setClickHeart(!clickHeart)}
          className={`${
            clickHeart ? "text-red-800" : "text-gray-400"
          } cursor-pointer`}
        />
        <IoBookmark
          onClick={() => setClickBookMark(!clickBookMark)}
          className={`${
            clickBookMark ? "text-green-500" : "text-gray-400"
          } cursor-pointer`}
        />
      </div>
      ...
      {/* Details section */}
      {/* Title section */}
      <div className="my-3 px-2 flex justify-between items-end">
        <div>
