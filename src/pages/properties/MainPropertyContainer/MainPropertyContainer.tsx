...
const MainPropertyContainer = ({setId}: {setId: React.Dispatch<React.SetStateAction<number>>}) => {

  return (
    <div className="w-[60%] bg-white rounded-2xl p-2">
      <div className="grid grid-cols-4 gap-5">
        {properties.map((property) => (
          <PropertyCard key={property.id} id={property.id} imageURL={property.imageUrl} title={property.title} location={property.location} price={property.price} ratings={property.ratings} type={property.type} setId={setId} />
        ))}
      </div>
    </div>
  );
